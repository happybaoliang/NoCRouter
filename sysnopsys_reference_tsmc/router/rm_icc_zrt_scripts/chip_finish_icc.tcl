...
###################################################
#Ted's changes
#-timeing report sort by slack instead of group
#-
###################################################
...
if {$ICC_REPORTING_EFFORT != "OFF" } {
 redirect -file $REPORTS_DIR_CHIP_FINISH/$ICC_CHIP_FINISH_CEL.max.tim {report_timing -crosstalk_delta -capacitance -transition_time -input_pins -nets -delay max -path full_clock -sort_by slack -max_paths 10} 
 redirect -file $REPORTS_DIR_CHIP_FINISH/$ICC_CHIP_FINISH_CEL.min.tim {report_timing -crosstalk_delta -capacitance -transition_time -input_pins -nets -delay min -path full_clock -sort_by slack -max_paths 10} 
}
